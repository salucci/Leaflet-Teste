...
if ($result = $mysqli->query("SHOW TABLES LIKE '".session_id().$table."'")) 
{
	if($result->num_rows == 0) 
	{
			...
			$sql = "insert into tablecontrol(SessionId,TableName,CreateTime,Type,user) values('".session_id()."','".$table."','".date('Y-m-d H:i:s',$_SESSION['CREATED'])."','".$_POST["options"]."',".$_SESSION['id'].")";
			echo $sql; 
			$tcresult = $mysqli->query($sql) or die(mysqli_error($mysqli));
	}
	else echo "Existe uma tabela ativa com este nome nesta sessão, por favor altere o nome do arquivo e tente novamente";
}
